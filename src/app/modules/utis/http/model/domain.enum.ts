...
export namespace Domain {
  export function getValues(): Array<Domain> | string[]{
    const array = [];
  ...
  export function serialize(domain: Domain | string): string | null {
  ...
  export function deserialize(domain: Domain | string): Domain | null {
    if (isNullOrUndefined(domain)) return null;
    switch (domain) {
      case Domain.PUBLIC:
      case Domain[Domain.PUBLIC]:
